...
from .lr_warmup import GradualWarmupScheduler
...
def Cosine(optim,
    ...
    warmup_epoch = round(warmup_epoch * step_each_epoch)

    if last_epoch == -1 and warmup_epoch > 0:
        scheduler_steplr = CosineAnnealingLR(optim, T_max=T_max)
        ...
        scheduler_steplr = CosineAnnealingLR(optim,
                                             T_max=T_max,
                                             last_epoch=last_epoch)
...
def Piecewise(optim,
    ...
    if last_epoch == -1:
        scheduler_steplr = MultiStepLR(optim,
