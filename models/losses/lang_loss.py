...
class LangMultiLosses(nn.Module):
    ...
    def _ce_loss(self, output, batch, idx=None, record=True):
        gt_labels, gt_lengths = batch[2], batch[3]
...
class SoftCrossEntropyLoss(nn.Module):
    ...
    def forward(self, input, target, softmax=True):
        if softmax: log_prob = nn.functional.log_softmax(input, dim=-1)
