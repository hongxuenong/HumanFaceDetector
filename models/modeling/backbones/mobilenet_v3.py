...
class MobileNetV3(nn.Module):
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.Connectors = nn.ModuleList([self.build_feature_connector(t, s) for t, s in zip([128,256,512,1024], [24,40,112,960])])
        ...
        self.out_channels = [128,256,512,1024] # use teacher out_channels
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        # transform to teacher feature
        new_out_list = []
        if self.Connectors is not None:
            for i in range(len(out_list)):
                new_out_list.append(self.Connectors[i](out_list[i]))
        return new_out_list
...
if __name__ == "__main__":
    ...
    # model = MobileNetV3(in_channels = 3).cuda()
    model = mobilenet_v3_large().cuda()
    weight = torch.load(
        './pretrained_weights/mobilenet_v3_pytorch_pretrained.ptparams')
