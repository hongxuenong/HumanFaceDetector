...
from ..utils.weight_init import trunc_normal_, constant_init, trunc_normal_init

# from .utils import get_root_logger
from ..utils.transformer import PatchEmbed, PatchMerging, to_2tuple, build_norm_layer, FFN, build_dropout
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        for i in out_indices:
            ...
            self.out_channels.append(self.num_features[i])
    ...
    def init_weights(self):
        if self.pretrained is None:
            ...
            #     # logger=logger,
            #     map_location='cpu')
            # print(ckpt['state_dict']['state_dict'])
            ...
            for table_key in relative_position_bias_table_keys:
                ...
                if nH1 != nH2:
                    ...
                    print('Error in loading {table_key}, pass')
                elif L1 != L2:
                    S1 = int(L1**0.5)
                    S2 = int(L2**0.5)
    ...
    def forward(self, x):
        ...
        for i, stage in enumerate(self.stages):
            x, hw_shape, out, out_hw_shape = checkpoint(
                stage, x, hw_shape) if self.use_checkpoint else stage(
                    x, hw_shape)
...
if __name__ == "__main__":
    ...
    model = SwinTransformer(
        ...
        num_heads=[4, 8, 16, 32],
        out_indices=(0, 1, 2, 3),
        pretrained='triton-det/saved_models/dbnetpp_small_best.pth').cuda()
    model.init_weights()
    # summary(model, input_size=(1, 3, 384, 384))
    # print(model)
    arr = torch.ones((1, 3, 384, 384)).cuda()
    model.eval()
    out = model(arr)
    print(out[0])