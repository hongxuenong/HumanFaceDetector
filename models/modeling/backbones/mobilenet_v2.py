...
class MobileNetV2(nn.Module):
    def __init__(
        ...
        features: List[nn.Module] = [
            Conv2dNormActivation(3, in_channels, stride=2, norm_layer=norm_layer, activation_layer=nn.ReLU6)
        ]
        ...
        for t, c, n, s in inverted_residual_setting:
            ...
            for i in range(n):
                ...
                features.append(block(in_channels, output_channel, stride, expand_ratio=t, norm_layer=norm_layer))
                in_channels = output_channel
        # building last several layers
        features.append(
            Conv2dNormActivation(
                in_channels, self.last_channel, kernel_size=1, norm_layer=norm_layer, activation_layer=nn.ReLU6
            )
        )
...
if __name__ == "__main__":
    ...
    model = mobilenet_v2(in_channels = 3).cuda()
    weight = torch.load(
        './pretrained_weights/mobilenet_v2_pytorch_pretrained.ptparams')
    # print(model.state_dict().keys())
    # model.load_state_dict(weight)
    # print(weight.keys())
    # summary(model, input_size=(1, 3, 640, 640), device="cuda:1")
    arr = torch.rand((8, 3, 640, 640)).cuda()
    out = model(arr)
