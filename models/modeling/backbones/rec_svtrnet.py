...
def drop_path(x, drop_prob=0., training=False):
    ...
    output = x.divide(keep_prob) * random_tensor.cuda()
    return output
...
class SVTRNet(nn.Module):
    ...
    def forward(self, x):
        ...
        print('out forward_features:', np.sum(out), np.mean(out), np.max(out), np.min(out))
        if self.use_lenhead:
        ...
        if self.last_stage:
            ...
            x = self.dropout(x)
            out = x.data.cpu().numpy()
            print('out last_stage:', np.sum(out), np.mean(out), np.max(out), np.min(out))
        if self.use_lenhead:
            return x, len_x
        return x


def load_paddle_weights(model, path):
    import os
    import paddle
    import paddle.fluid as fluid
    with fluid.dygraph.guard():
        para_state_dict, _ = fluid.load_dygraph(path)
...
if __name__ == "__main__":
    ...
    np.random.seed(666)
    set_seed(0)
    inputs = np.random.randn(1, 3, 32, 100).astype(np.float32)
    inp = torch.from_numpy(inputs).cuda()
    model = SVTRNet(prenorm=False).cuda()
    load_paddle_weights(model, '/ldap_home/jingze.chi/PaddleOCR/pretrained/rec_svtr_tiny_none_ctc_en_train/best_accuracy')
    # summary(model, input_size=(1, 3, 32, 100), device="cuda")
    out = model(inp)
