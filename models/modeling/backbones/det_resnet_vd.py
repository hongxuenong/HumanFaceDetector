...
class ResNet(nn.Module):
    ...
    def forward(self, y):
        # y = torch.Variable(y,requires_grad=True) 
        if self.use_checkpoint:
            y = checkpoint(self.conv1_1,y)
            y = checkpoint(self.conv1_2,y)
            y = checkpoint(self.conv1_3,y)
            y = checkpoint(self.pool2d_max,y)
            out = []
...
if __name__ == "__main__":
    ...
    model = ResNet(in_channels=3,disable_se=True).cuda()
    # summary(model, input_size=(1, 3, 640, 640), device="cuda:0")
