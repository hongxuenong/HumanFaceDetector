    ...
    def forward(self, feat, img_metas=None):
        ...
        if valid_ratios is not None:
            ...
            for i in range(valid_ratios.shape[0]):
                valid_step = min(T, torch.ceil(valid_ratios[i] * T).type(torch.long)) - 1
    ...
    def _2d_attention(self,
        ...
        if valid_ratios is not None:
            ...
            for i in range(valid_ratios.shape[0]):
                valid_width = min(
                    w,
                    torch.ceil(valid_ratios[i] * w).type(torch.long))
                if valid_width < w:
...
class SARHead(nn.Module):
    ...
    def __init__(self,
        ...
        # decoder module
        self.decoder = ParallelSARDecoder(out_channels=out_channels,
...
if __name__ == "__main__":
    ...
    arr = torch.rand((1, 512, 1, 40))
    model = SARHead(512, 3002, max_text_length=25)
    # print(model)
    # for i, key in enumerate(model.state_dict()):
    #     print("{}  {}  {}".format(i, key, model.state_dict()[key].size()))
    summary(model, input_size=(1, 512, 1, 40), device="cpu")
    # out = model(arr)
    # print(out.size())
